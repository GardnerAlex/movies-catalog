...
// @ts-ignore
import { BASE_BACKDROP_PATH, BASE_POSTER_PATH } from '../../constants/Constants';
...
export class MovieDetails extends Component {
  ...
  render() {
    ...
    if (movieReviews && movieReviews.length > 2) {
      ...
      reviews = prevReviews.map((review: { id: any; author: any; }) => (
        <Review key={review.id} author={review.author} review={review} />
      ));
    } else if (movieReviews && movieReviews.length < 2) {
      reviews = movieReviews.map((review: { id: any; author: any; }) => (
        <Review key={review.id} author={review.author} review={review} />
      ));
    }
